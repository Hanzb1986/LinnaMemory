...
> ¹ XLSX 仅当 韩工 明确要求 Excel 时使用；默认表单/数据类输出 CSV（UTF-8），详见下方格式标准表。
...
| ③ | **工作表单/任务清单/数据表格** | **`.csv`** | **¥ 韩工 新规 2026-07-07：一律 CSV，UTF-8 编码，WPS/Excel 直接双击打开。除非 韩工 明确要 Excel，否则不出 .xlsx。** |
| ④ | 可编辑底稿/台账/报表/清单 | `.xlsx` | **仅当 韩工 明确要求 Excel 时使用** |
...
**cron：** ❌ 已禁用（2026-07-08 起，高频 agentTurn 违规，见 01_BUSINESS_CONFIG §15.3）
**实时规则：** 每次重要的配置/文件变更后，会话内手动 git push（唯一通道）
